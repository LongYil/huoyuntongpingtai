...
public class AssembleJyjl {
	...
	public List<JyjlVo> getCzsq(List<Jyjl> list) throws Exception{
		listvo.clear();
		for(int i=0;i<list.size();i++){
			jyjl = list.get(i);
			jbyh = new Jbyh();
			jyjlvo = new JyjlVo();
			ptzh = ptzhServc.find(String.valueOf(list.get(i).getYhbh()));
			jyjlvo.setPtzh(ptzh);
	...
	public List<JyjlVo> getTxsq(List<Jyjl> list) throws Exception{
		listvo.clear();
		for(int i=0;i<list.size();i++){
			jyjl = list.get(i);
			jbyh = new Jbyh();
